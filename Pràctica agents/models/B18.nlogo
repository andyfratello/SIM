...
to setup
  ...
  create-Talps TalpsTotal[
    ...
    set Fertil false
    set target nobody
    set Radi 17 - Velocitat
    set Copulacio false
    set PerritoGuardian 0
  ]
...
to calcul-punt-secret
  let x random-xcor
  let y random-ycor
  ifelse [pcolor] of patch x y != 95 [
    set Xsecret x
    set Ysecret y
  ]
  [
...
to setxy-talp
  setxy random-xcor random-ycor
  if ([pcolor] of patch xcor ycor = 95)[
    setxy-talp
  ]
end
    ...
    ifelse Vida > 249[
      ...
      [
        ...
        [
          ...
          [
            let aigue [aigua] of target
            if (distance target > Radi) or (aigue)
            [ set-target-talp ]
          ]
...
to talp-copula
  if  (any? other Talps-here with [Copulacio = false and Fertil = true]) and Copulacio = False[
    set Copulacio  True
    set PerritoGuardian 230
    let parella one-of other Talps-here with [Copulacio = false and Fertil = true]
    ask parella [
      set Copulacio True
      set PerritoGuardian 230
    ]
...
to talp-envelleix
  ...
  if PerritoGuardian > 0 [
    set PerritoGuardian PerritoGuardian - 1
  ]
    if Vida >= VidaMax or Inanició <= 0 [
      die
    ]
end

to talp-morir [qui]
  ifelse(([breed] of qui = Granotes and Vida < 250) or ([breed] of qui = Guineus) and (count Talps-here < 5) and [cau] of patch-here != true ) [
    set color red
    die
  ]
  [
...
to set-target-talp
  ifelse Fertil = true
  [
    ifelse any? other Talps in-radius Radi with [Copulacio = false and Fertil = true]
    [
      set target one-of other Talps in-radius Radi with [Copulacio = false and Fertil = true]
      let copula [Copulacio] of target
      if(copula) [set target nobody]
    ]
    [
      set target nobody
    ]
  ...
  [
    set target nobody
    if any? Papallones in-radius Radi
    [
      set target one-of Papallones in-radius Radi
      let aigue [aigua] of target
      if(aigue) [set target nobody]
    ]
    if any? Granotes in-radius Radi
    [
      set target one-of Granotes in-radius Radi
      let aigue [aigua] of target
      if(aigue) [set target nobody]
    ]
    if any? Aranyes in-radius Radi
    [
      set target one-of Aranyes in-radius Radi
      let aigue [aigua] of target
      if(aigue) [set target nobody]
    ]
  ]
...
to talp-menja
  if breed = Talps and ((count Aranyes-here > 0 and count Aranyes-here < 6) or (count Papallones-here > 0 and count Papallones-here < 6) or (count Granotes-here > 0 and count Granotes-here < 6)) and Vida > 250
  [
...
to moure-random
  ...
  ifelse patch-ahead 2 != nobody [
    ifelse [pcolor] of patch-ahead 2 = 95 [
      rt random-float 200 - random-float 30
      moure-random
    ]
    [
      fd 2
