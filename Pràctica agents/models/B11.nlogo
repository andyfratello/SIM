...
to setup
  ...
  create-Guineus GuineusTotal[
    guineu-setCoords
    ifelse not any? Carnivores-on patch guineu-generax guineu-generay  [
      setxy guineu-generax guineu-generay
    ]
    [
      guineu-setCoords
    ]
    ...
    set cicles-menjant 0
    set estic-menjant false
    set cicles-gestació 0
    set estic-gestant false
  ]
...
to guineu-setCoords
  set guineu-generax random-xcor
  set guineu-generay random-ycor
  ask patch guineu-generax guineu-generay [
    if Aigua = true [
      guineu-setCoords
    ]
  ]
...
to go
  ...
  ask Guineus [
    ...
    if guineu-pot-moure[
      guineu-unir-dos
      guineu-gestionar-moviment
    ]
  ]
  tick
end

to guineu-gestionar-moviment
  let prioritat ""
  ifelse (Fertilitat > 9950 and Fertilitat < 10100) or (Fertilitat > 19950 and Fertilitat < 20100) [
    set prioritat "buscarParella"
  ]
  [
    ifelse Inanició > 100 and prioritat != "buscarParella"[
      set prioritat "caçarMenjar"
    ]
    [
      set prioritat "moureRandom"
    ]
  ]
  ifelse prioritat = "caçarMenjar" [
    guineu-caçar
  ]
  [
    ifelse prioritat = "buscarParella" [
      guineu-unir-dos
    ]
    [
      guineu-moure-random
...
to guineu-unir-dos
  ;si encara no te parella, assigna la mes propera
  if count Guineus > 1 [
    if not any? my-links[
      create-link-with min-one-of other Guineus [distance myself]
    ]
    let parella one-of link-neighbors
    face parella
    guineu-gestionar-aparellament
  ]
end

to guineu-caçar
  let presa one-of Talps in-radius 9
  set presa min-one-of Talps [distance myself]
  ifelse presa != nobody [
    face presa
    ifelse distance presa = 0 [
      ask presa[morir]
      set Inanició 0
      set estic-menjant true
    ]
    [
        let destinationX [xcor] of presa
        let destinationY [ycor] of presa
        let wat  [aigua] of patch destinationX destinationY
        let pc one-of Carnivores-at destinationX destinationY
        ifelse wat = false and pc = nobody[
          move-to presa
          ask presa[morir]
          set Inanició 0
          set estic-menjant true
  ...
  [
    let presa2 one-of Granotes in-radius 9
    set presa2 min-one-of Granotes [distance myself]
    ifelse presa2 != nobody [

      face presa2
      ifelse distance presa2 = 0 [
      ...
      [
        ...
        let destinationX2 [xcor] of presa2
        let destinationY2 [ycor] of presa2
        let wat2  [aigua] of patch destinationX2 destinationY2
        let pc2 one-of Carnivores-at destinationX2 destinationY2
        ifelse wat2 = false and pc2 = nobody[
...
to guineu-estic-viu
  ...
  if Vida = 25000 or Inanició = 1000[die]
  if MaduresaCries < 500 [set MaduresaCries MaduresaCries + 1]
  if MaduresaCries = 500 [set MaduresaCries 501]
  if Fertilitat >= 10000 [
    set PeriodeFertilitat PeriodeFertilitat + 1
    if PeriodeFertilitat = 100 [
      set PeriodeFertilitat 0
      set Fertilitat 0
    ]
  ]
...
to guineu-gestionar-aparellament
  let cX [xcor] of self
  let cY [ycor] of self
  if not empty? filter [i -> i = true] [any? Guineus-here] of neighbors[
    let parella filter [i -> i != nobody] [one-of Guineus-here] of neighbors
    let parella-fertilitat [Fertilitat] of first parella
    let parella-periodeFertilitat [PeriodeFertilitat] of first parella
    if Fertilitat = 10000 and parella-Fertilitat = 10000 and PeriodeFertilitat < 100 and parella-periodeFertilitat < 100 [
      hatch-Guineus 3 [
        setxy cX cY
        set shape "wolf"
        set color blue
        ...
        set cicles-gestació 0
        set estic-gestant false
        create-link-with first parella [tie hide-link]
      ]
    ]
  ]
end

to-report guineu-pot-moure
  ifelse estic-menjant and cicles-menjant < 25 [
    set cicles-menjant cicles-menjant + 1
    if cicles-menjant = 25 [
      set cicles-menjant 0
      set estic-menjant false
      set Inanició 0
    ]
  ...
  [
    ifelse estic-gestant and cicles-gestació < 100 [
      set cicles-gestació cicles-gestació + 1
      if cicles-gestació = 100 [
        set cicles-gestació 0
        set estic-gestant false
      ]
      report false
...
to moure-generic ; turtle procedure
   ifelse breed = Flors or breed = Carnivores or breed = Guineus[
  ]
  [
...
to guineu-moure-random
 let d random 9
 let r random 360
 ifelse patch-at-heading-and-distance r d != nobody[
    let wat [aigua] of patch-at-heading-and-distance r d
    ifelse not wat and not any? Carnivores-on patch-at-heading-and-distance r d[
      set heading r
      fd d
    ]
    [
