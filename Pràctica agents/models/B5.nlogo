...
to setup
  ...
  create-Aranyes AranyesTotal[
    ...
    set Creades Creades + 1
    setxy aranya-PosX aranya-PosY
    set shape "spider"
    set color blue
    set size 6
    set Menjant 0
    ...
    set Inanició 0
    set Fertilitat 0
    set teranyina-feta false
    set aranya-lligada false
    set aranya-teixint false
    set xteranyina 0
    set yteranyina 0
    set aranya-moguda false
...
to go
  ...
  ask Aranyes [
    if aranya-lligada = false [
      set aranya-lligada true
      let lligades 0
      while [count Aranyes-here with [aranya-lligada = false] > 0 and lligades < 10]
      [
        let candidat one-of Aranyes-here with [aranya-lligada = false]
        create-link-with candidat [tie]
        ask candidat [
          set aranya-lligada true
        ]
        set lligades lligades + 1
      ]
    ]
    estic-viu
    if es-cicle-valid ;
    [
      ;Fes creixer a les aranyes
      if Vida = 200 [
        set size 6
      ]
      if Vida > 1000 and not teranyina-feta [
        fes-teranyina
      ]
      if not aranya-teixint and count Aranyes-here with [aranya-teixint = true] = 0 [

        if count Aranyes-here with [aranya-moguda] = 0[
          ens-movem ;
        ]
        set aranya-moguda true ;

        ; Ja s'han aranya-moguda totes i es resetejen
        if count Aranyes-here with [aranya-moguda] = count Aranyes-here [
          ask Aranyes-here [ set aranya-moguda false]
        ]
        ens-reproduim ;
      ]
    ]
...
to ens-movem
  ifelse Vida < 1000 or Vida > 1500 and Vida < 1625 [
    if any? neighbors with [Terra = true or Teranyina = true] [
      move-to one-of neighbors with [Terra = true or Teranyina = true]
    ]
  ]
  [
    ifelse any? neighbors with [Aigua = true] [
      move-to one-of neighbors with [Aigua = true]
    ]
    [
      move-to one-of neighbors with [Terra = true or Teranyina = true]
    ]
...
to ens-reproduim
  if count Aranyes-here mod 2 = 0 and Fertilitat > 800 and Fertilitat < 825 and who mod 2 = 0 and Terra = true or Teranyina = true [
    set Fertilitat 0
    hatch 100 [
      set size 3
      set Vida 0
  ...
  if Vida > 700 and estic-menjant = false [
    ; Menjar cria
    set estic-menjant true
    set Inanició 0
    let cria one-of Aranyes-here with [ Vida = 0 ]
    ifelse cria = nobody [
    ][
      ask cria[ morir-aranyes myself]
    ]
  ]
...
to fes-teranyina
  ...
  if aranya-teixint = false and Aigua and teranyina-feta = false and count Aranyes-here with [aranya-teixint = true] = 0 [
    if pxcor + 5 < max-pxcor and pycor + 5 < max-pycor [
      set aranya-teixint true
      set xteranyina xcor
      set yteranyina ycor
      set Teranyina true
      set pcolor 9
...
to estrategia-teranyina
  if (ycor - yteranyina) mod 2 = 0 and xcor < xteranyina + 5 [
    set xcor xcor + 1
    stop
  ]
  if (ycor - yteranyina) mod 2 = 1 and xcor > xteranyina [
    set xcor xcor - 1
    stop
  ]
  if (xcor = (xteranyina + 5) or xcor = xteranyina) and ycor < (yteranyina + 4) [
    set ycor ycor + 1
    stop
  ]
  if ycor = yteranyina + 4 and xcor = xteranyina + 5 [
    set teranyina-feta true
    set aranya-teixint false
    set xteranyina 0
    set yteranyina 0
    stop
...
to estic-viu
  ...
  if Vida = 2000 or Inanició = 1500 [
    die
  ]
end

to-report es-cicle-valid
  ifelse estic-menjant and Menjant < 5 [
...
to morir-aranyes [qui]
  ...
  ifelse count Aranyes-here >= 5 and Vida > 1000 [
    ask qui [penalitzat]
  ]
  [
    if count Aranyes-here >= 5 and Vida > 1000 and qui = Fenomens [
      morir

      if qui = Talps or qui = Granotes or qui = Carnivores [
        ifelse count Aranyes-here >= 5 [
          ask qui[penalitzat]
        ]
        [
          morir
      ...
      if qui = Guineus or qui = Papallones or qui = Flors [
        ask qui [penalitzat]
      ]
    ]
  ]
...
to setCoords
  set aranya-PosX random-xcor
  set aranya-PosY random-ycor
  ask patch aranya-Posx aranya-Posy [
    if Aigua = true [
      setCoords
    ]
  ]
