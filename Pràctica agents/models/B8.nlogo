...
to talp-posicion
  let x 0
  let y 0
  let p one-of Patches with [ (aigua = false) and ([count Patches in-radius 100 with [ aigua = true ]] of self = 0)]
  if p = nobody [
    set p one-of Patches with [ (aigua = false) and ([count Patches in-radius 25 with [ aigua = true ]] of self = 0)]
  ]
  ifelse p != nobody [
    ask p [
      set x pxcor
...
to go
  ...
  ask Talps [
    ;DEFINIR EL RADIO DEL CAMPO DE VISION
    let tradius 17 - Velocitat

    ;MIRAMOS SI TIENE QUE MORIR
    if(Aigua)[ die ]
    if tvida = Vida [ die ]
    if tinanició = Inanició [ die ]

    ;SI ESTAS COMIENDO SIGUE COMIENDO O ACABA
    if tcomiendo > 0 [
      set tcomiendo tcomiendo - 1
      if tcomiendo = 0 [set tinanició 0]
    ]

    ;SI ERES UNA CRIA
    if tcria < MaduresaCries [
      set tcria tcria + 1
      set tvida tvida + 1
      if tcria = MaduresaCries [ set color black ]
    ]

    ;SI NO ERES UNA CRIA
    if tcria = MaduresaCries [
      ;SI ESTAS EN UN PERIODO DE FERTILIDAD
      if tperiodefertilitat >= 0 [
        ;SI ENCUENTRA PAREJA EN SU POSICION
        if count Talps-here with [tperiodefertilitat >= 0 and tparella = false] > 1 [
          ask one-of other Talps-here with [tperiodefertilitat >= 0 and tparella = false] [
            ;LOS EMPAREJA
            set tparella True
            ask myself [set tparella True]
            ;UNO DE ELLOS HACE EL CAU
        ...
        ;SI NO TIENES PAREJA SIGUE BUSCANDO
        ifelse tparella = false [
          let target one-of other Talps in-radius tradius with [ tperiodefertilitat >= 0 and tparella = false]
           ;MOVERSE 1 O 2 DEPENDIENDO DE COMO DE LEJOS ESTE
          ifelse target != nobody [
            face target
            ifelse distance target = 1 [talp-moure1][talp-moure]
          ][
            ifelse tcaux != 0 [facexy tcaux tcauy][rt random-float 90 - random-float 90]
            talp-moure
          ]
        ][
          ;SI TIENES PAREJA COME O BUSCA COMIDA
          talp-menja
          let t turtles-on patches with [aigua = false] in-radius tradius
          let target min-one-of t with [breed = Granotes or breed = Papallones or breed = Aranyes] [distance myself]
          let depredador one-of Guineus in-radius tradius
          ;MOVERSE 1 O 2 DEPENDIENDO DE COMO DE LEJOS ESTE
          ifelse target != nobody [
            face target
            ifelse distance target = 1[talp-moure1][talp-moure]
          ...
          [
            ifelse depredador != nobody [
              let segura nobody
              set segura min-one-of Carnivores in-radius tradius [distance myself]
              if segura = nobody [set segura min-one-of patches in-radius tradius with [ cau = true ][distance myself]]
              if segura != nobody [
                face segura
                talp-moure
              ]
        ...
        ;ACTUALIZAMOS PERIODO DE FERTILIDAD
        set tperiodefertilitat tperiodefertilitat + 1

        ;SI HA ACABADO REINICIAMOS VALORES
        if tperiodefertilitat = PeriodeFertilitat [
          set tparella false
          set tperiodefertilitat -1
        ]
      ]
      ;SI NO ESTAS A UN PERIODE DE FERTILITAT
      if tperiodefertilitat < 0 [
        ;COME O BUSCA COMIDA
        talp-menja
        let t turtles-on patches with [aigua = false] in-radius tradius
        ...
        ifelse target != nobody [
          ...
          ifelse depredador != nobody [
            let segura nobody
            set segura min-one-of patches in-radius tradius with [ cau = true ][distance myself]
            if segura = nobody [set segura min-one-of Carnivores in-radius tradius [distance myself]]
            if segura != nobody [
              face segura
              talp-moure
            ]
          ][
            ifelse tfertilitat > 800 and tcaux != 0 [facexy tcaux tcauy][rt random-float 90 - random-float 90]
            talp-moure
          ]
        ]
        talp-menja
      ...
      set tvida tvida + 1
      set tinanició tinanició + 1
      set tfertilitat tfertilitat + 1
      if tfertilitat = Fertilitat [
        set tperiodefertilitat tperiodefertilitat + 1
        set tfertilitat 0
      ]
    ]
...
to talp-moure ;
  ...
  if patch-ahead Velocitat != nobody [
    ask patch-ahead Velocitat [
      if aigua = true [
        ask myself [ rt 90 ]
      ]
    ]
  ]
  if Cau and count Carnivores in-radius 6 >= 4 [set p true]
  if tcomiendo > 0 [ set p true ]
  if p = false [ fd Velocitat ]
end
...
to talp-menja ;
  if tcomiendo = 0 and count Aranyes-here >= 1  and (count Aranyes-here + count Guineus-here + count Papallones-here + count Flors-here) < 5 [
    ask one-of Aranyes-here[ morir-aranya myself ]
    set tcomiendo 5
    set tinanició 0
  ]
  if tcomiendo = 0 and count Papallones-here >= 1  and (count Papallones-here + count Guineus-here + count Flors-here) < 5 [
    ask one-of Papallones-here[ morir-papallona myself ]
    set tcomiendo 5
    set tinanició 0
  ]
  if tcomiendo = 0 and count Granotes-here >= 1  and (count Granotes-here + count Aranyes-here + count Carnivores-here + count Papallones-here + count Flors-here) < 5 [
    ask one-of Granotes-here[ morir-granota myself ]
    set tcomiendo 5
    set tinanició 0
  ]
end
...
to morir-talp [qui]
  ifelse qui != Guineus or qui != Granotes [
    ask qui [ penalitzats ]
  ]
  [
    if qui = Granotes [
      ifelse tcria = MaduresaCries or (count Talps-here + count Aranyes-here + count Carnivores-here + count Papallones-here + count Flors-here + count Granotes-here) >= 5[
        ask qui [ penalitzats ]
      ][ die ]
    ]
    if qui = Guineus [
      ifelse (count Talps-here + count Aranyes-here + count Carnivores-here + count Papallones-here + count Flors-here + count Granotes-here) >= 5  [
        ask qui [ penalitzats ]
      ][ die ]
    ]
  ]
