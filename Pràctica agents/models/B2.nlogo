...
to setup
  ...
  create-Carnivores CarnivoresTotal[
    let aux (list carnivora-getpos)
    let coord item 0 aux
...
to go
  ask turtles [
    ...
    carnivora-check-reproduce
    carnivora-check-auto-death
    carnivora-update
  ]
  tick
end
...
to carnivora-check-reproduce

  ask carnivores[
    if(PeriodeFertilitat <= 0)[
      let aux (list carnivora-coord-reproduccion xcor ycor 1)
      let coord item 0 aux
      let x1 item 0 coord
      let y1 item 1 coord

      set aux (list carnivora-coord-reproduccion xcor ycor 2)
      set coord item 0 aux
      let x2 item 0 coord
      let y2 item 1 coord

      set aux (list carnivora-coord-reproduccion xcor ycor 3)
      set coord item 0 aux
      let x3 item 0 coord
      let y3 item 1 coord
      set PeriodeFertilitat 30000
      if (x1 != 999999999999999 and y1 != 999999999999999)[
        hatch 1 [
          set Vida 90000
          set PeriodeFertilitat 30000
          setxy x1 y1
      ...
      if (x2 != 999999999999999 and y2 != 999999999999999)[
        hatch 1 [
          set Vida 90000
          set PeriodeFertilitat 30000
          setxy x2 y2
      ...
      if (x3 != 999999999999999 and y3 != 999999999999999)[
        hatch 1 [
          set Vida 90000
          set PeriodeFertilitat 30000
          setxy x3 y3
...
to-report carnivora-coord-reproduccion[x y n]
  let aux (list carnivora-coords-calcul x y n)
  let coord item 0 aux
  set x item 0 coord
  set y item 1 coord

  if(carnivora-space-available x y = false)
  [
    set x 999999999999999
    set y 999999999999999
  ]
  report list (x) (y)
end

to-report carnivora-coords-calcul [x y n]
  if (n = 1) [
   set x x - 5
  ]
  if (n = 2) [
   set y y - 5
   if ( carnivora-space-available x y = false) [set y y + 10]
  ]
  if (n = 3) [
   set x x + 5
  ]
...
to-report carnivora-in-limits [x y]
  ifelse x > min-pxcor and x < max-pxcor and y > min-pycor and y < max-pycor
  [report true]
  [report false]
end

to-report carnivora-space-available [x y]
  ifelse carnivora-in-limits x y
  [
    ifelse any? other carnivores-on patch x y or [aigua] of patch x y or count carnivores in-radius 4 > 1
    [report false]
    [report true]
  ]
  [report false]
...
to eat-generic
  ...
  [
    ...
    ifelse (count Aranyes-here > 0)[
      let presa one-of Aranyes-here
      ask presa [aranya-morir]
      beep
    ]
    [
      let presa one-of Papallones-here
      ask presa[papallona-morir]
      beep
    ]
  ]
