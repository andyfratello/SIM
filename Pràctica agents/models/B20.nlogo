...
to setup
  ...
  create-Papallones PapallonesTotal[
    ...
    set Vida 5000
    set velocitat 5
    set ticksInanicio 1000
    set ticksGestacio 0
    set ticksMenjar 0
    set ticksBuscarParella 1000
    set Partner nobody
  ]
...
to go
  ask turtles with [breed != Papallones][
    moure-generic
    eat-generic
  ]
  ask Papallones
  [
    velocitat-papallones
    if ticksMenjar = 0
    [
      moure-papallones
    ]
    mata-papallones
    setTicks-papallones
    if ticksFenomenmisterios = 0 and Vida < 100 and Vida != false[
      fenomen-misterios
    ]
  ]
...
to setTicks-papallones
  if ticksMenjar > 0 [
    set ticksMenjar ticksMenjar - 1
  ]

  if Vida > 0 or Vida != false [
    set Vida Vida - 1
  ]

  if ticksCria > 0 [
    set ticksCria ticksCria - 1
  ]

  if ticksInanicio > 0 [
    set ticksInanicio ticksInanicio - 1
  ]
  if ticksBuscarParella > 0 [
    set ticksBuscarParella ticksBuscarParella - 1
  ]

  if ticksGestacio > 0 [
    set ticksGestacio ticksGestacio - 1
  ]

  if ticksTeranyina > 0 [
    set ticksTeranyina ticksTeranyina - 1
  ]

  ifelse ticksLarva > 0 [
    set ticksLarva ticksLarva - 1
  ]
  [
    evoluciona-papallones
  ]

  if ticksFenomenmisterios > 0 [
    set ticksFenomenmisterios ticksFenomenmisterios - 1
  ]
end

to velocitat-papallones
  set velocitat 1
end
...
to buscar-aigua
  ...
  let candidatesPatches patches in-radius (17 - velocitat) with [ aigua = true ]
  ifelse any? candidatesPatches[
    let target with [distance myself <= 15] max-one-of candidatesPatches [distance myself]
    move-to target
  ]
  [
    let candidatesTargets patches in-radius (17 - velocitat) with [ teranyina = false ]
    ifelse any? candidatesTargets[
      let target with [distance myself <= 15] one-of candidatesTargets
      face target
      move-to target
    ]
    [
...
to moure-papallones
  ifelse ticksLarva > 0 or ticksTeranyina > 1[
  ][
    ifelse ticksInanicio < 400 and ticksMenjar = 0 and ticksBuscarParella > 0[;Necessita menjar
      buscar-flors
    ]
    [;necessita reproduir-se
      ifelse ticksBuscarParella = 0[
        buscar-parella
      ]
      [
       buscar-aigua
  ...
  let teranyinaHere false
  if ticksTeranyina = 0[
    ask patch-here[
      if teranyina = true[
        set teranyinaHere true
      ]
    ]
    if teranyinaHere[
      set TicksTeranyina 101
    ]
  ]
...
to buscar-flors
  let candidatesFlors Flors in-radius (17 - velocitat)
  ifelse any? candidatesFlors [
    if any? Flors-here[
      eat-papallones
    ]
    let target with [distance myself <= velocitat] min-one-of candidatesFlors [distance myself]
    move-to target
  ]
  [
    rt random-float 90 - random-float 90
...
to buscar-parella
  ifelse (partner = nobody) and (any? other Papallones in-radius (17 - velocitat) with [partner = nobody])[
    let partnerPapallona min-one-of other Papallones in-radius (17 - velocitat) with [partner = nobody] [distance myself]
    set partner partnerPapallona
    ask partnerPapallona [
      set partner myself
    ]
    set ticksBuscarParella 1000
    set ticksGestacio 10
    move-to partnerPapallona
  ]
  [
    rt random-float 90 - random-float 90
    fd velocitat
...
to mata-papallones
  if (Vida = 0 or ticksInanicio = 0) and Vida != false[
    die
  ]
end
...
to evoluciona-papallones
  set shape "butterfly"
end


to fenomen-misterios
  let candidatesLarves Papallones in-radius (17 - velocitat) with [ticksLarva < 0]
  if any? candidatesLarves [
    let target one-of candidatesLarves
    ask target[
      morir
    ]
    set ticksFenomenMisterios 1000
    set Vida false ;immortal
  ]
end
