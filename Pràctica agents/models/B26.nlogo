...
to decrementar-granota
  ...
  if (menjant > 0)  [ set menjant (menjant - 1)]
  if (ciclosFertilizando > 0) [ set CiclosFertilizando (CiclosFertilizando - 1)]

  if (protegint = 0) and (menjant = 0) and (ciclosFertilizando = 0) [ set movimiento 1 ]
...
to granota-moure
  ...
  let myCiclosFertilizando CiclosFertilizando
  let myInanició inanició
  let myMovimiento movimiento
  let myMenjant menjant
  ...
  ifelse (myMovimiento = 1 or myProtegint > 0) [

    ask patches in-radius (17 - velocitatGranota) [ ; per tots els patch dins el radi...


      if(myInanició > 250) [
       if (socAlaigua = 1) and (count Granotes-here with [fertilitat > 0] > 0) and (fertilitatGranota > 0) and (myCiclosFertilizando = 0)[
          ...
          ask parella[ set gestacioParella CiclosFertilizando]
          ifelse (((distancexy myposx myposy) <= velocitatGranota) and (fertilitatParella > 0) and (myself != parella) and (gestacioParella = 0) and (pcolor = 95)) [
            ...
            set myCiclosFertilizando 100
            ask parella [
              set CiclosFertilizando 100
              set movimiento 0
            ]
          ]
          [
      ...
      if myInanició < 250  [
        ...
        if (count Aranyes-here > 0) and (count Aranyes-here < 5) and (distancexy myposx myposy) <= velocitatGranota and (myMenjant = 0)[
        ...
        if (count Talps-here with [vida >= 250] != 0) and (count Talps-here with [vida < 250] > 0) and (distancexy myposx myposy) <= velocitatGranota and (myMenjant = 0)[
        ...
        ifelse (posx = 0 and posy = 0 and rotate = -1) and (myProtegint = 0) and (count Papallones-here > 0) and (count Papallones-here < 5) and (pcolor != 95) and (myMenjant = 0) [
  ...
  [
    if ( myCiclosFertilizando - 1) = 0 [
      let padreMovimiento movimiento
      let padreVida vida
