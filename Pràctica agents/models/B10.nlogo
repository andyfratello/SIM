globals [
  ...
  guineu-patches-random
  guineu-patches-setup
  guineu-i
  guineu-patch-setup
  guineu-En-Periodo-Reproduccion
]
...
to setup
  clear-all
  reset-ticks
  set guineu-En-Periodo-Reproduccion []
  set guineu-patches-setup []
  set guineu-i 0
  ask patches
  [
    set father nobody
  ...
  create-Guineus GuineusTotal[
    ...
    if guineu-i = 0 [guineu-setup]
    ifelse not empty? guineu-patches-setup [
      if guineu-i mod 2 = 0[
        set guineu-patch-setup one-of guineu-patches-setup
      ]
      let x 0
      let y 0
      ask guineu-patch-setup [
        set x pxcor
        set y pycor
      ]
      setxy x y
      set guineu-i guineu-i + 1
    ]
    [
      let trobat false
      let x min-pxcor + 1
      let y min-pycor + 1
      while[not trobat AND x > min-pxcor AND x < max-pxcor AND y > min-pycor AND y < max-pycor] [
        let patch-candidat patch x y
        if [pcolor] of patch-candidat = 65 or  [pcolor] of patch-candidat = 9 and not any? Carnivores-on patch-candidat  [
          ;setxy x y
          set trobat true
        ]
        if not trobat [
    ...
    set Velocitat 8
    set Inanició 0
    set Fertilitat 10000
    set PeriodeFertilitat -1
    set Cries 3
    set MaduresaCries 500
    set PeriodeGestacio -1
    set TempsMenjant -1
    set EsCria false                 ;; Indica si es una cria
  ]
...
to guineu-setup
  ...
  ask patch-at 0 0[
      let p-pos patches in-radius 9
      if all? p-pos  [pcolor = 65 or pcolor = 9 and not any? Carnivores-here] [
        set guineu-patches-setup fput self guineu-patches-setup
      ]
  ]

  let x i * 9
  let z i * -9
  while [i < 55 and patch-at x x != nobody and patch-at x 0 != nobody ] [
    ask patch-at x x [
      let p-pos patches in-radius 9
      if all? p-pos  [pcolor = 65 or pcolor = 9 and not any? Carnivores-here] [
        set guineu-patches-setup fput self guineu-patches-setup
...
to go
  ask turtles [
    ...
    ifelse breed = Guineus [
      if (aigua) [ morir ]
      if (Vida = (25000 - Fertilitat)) or (Vida = (25000 - (Fertilitat + Fertilitat))) [;vida = 10000 or vida = 20000
        Guineu-Reset-PeriodeFertilitat
        set guineu-En-Periodo-Reproduccion lput self guineu-En-Periodo-Reproduccion
      ]
      ifelse EsCria = false OR Vida < (25000 - MaduresaCries)  ; Es adulto
      [
        ifelse (length  guineu-En-Periodo-Reproduccion) >= 2 AND member? self guineu-En-Periodo-Reproduccion [

          if Vida = 0 [ morir ]
          if Inanició = 1000 [ morir ]

          Guineu-Decrementa-Vida
          Guineu-Augmenta-Inanicio

          ifelse Vida < 5000 OR ( TempsMenjant >= 0 AND TempsMenjant < 25) [
            set TempsMenjant TempsMenjant + 1
          ]
          [
            if TempsMenjant = 25 [ set TempsMenjant -1 ] ; ha acabat de menjar


            let aux-list guineu-En-Periodo-Reproduccion
            set aux-list remove self aux-list
            let aux sort-by [ [a1 a2] -> distance a1 < distance a2 ]  aux-list
            let parella item 0 aux
            let distancia distance parella
            let patch-guineu patch-here
            let patches-guineu patches in-radius (distancia * 2)
            let p-valids patches-guineu with [pcolor = 65 or pcolor = 9 and not any? Carnivores-here  ]

            let patch-presa-guineu 0
            let pathfound  false
            let i 0
            while [not pathfound and i < length aux ] [
              ask item i aux[
                set patch-presa-guineu patch-here
              ]
              let path  guineu-A* patch-guineu patch-presa-guineu p-valids
              if path != false [
                set pathfound true
                let j 1
                while [j <= Velocitat and j < length path][
                  move-to item j path
                  set j j + 1
                ]
      ...
      [
        ifelse Madre != nobody [                          ;; Tiene madre
          guineu-moure-cria
        ]
        [
          guineu-moure
...
to guineu-moure-cria
  ...
  [
    if TempsMenjant = 25 [ set TempsMenjant -1 ]
    let distancia distance Madre
    if distancia > Velocitat [ set distancia Velocitat ]
    face Madre
    fd distancia
  ]
...
to guineu-moure
  ...
  if TempsMenjant = 25 [ set TempsMenjant -1 ]
  let patch-guineu patch-here
  let patches-guineu patches in-radius 9
  let p-valids patches-guineu with [pcolor = 65 or pcolor = 9 and not any? Carnivores-here  ]
  let encerrado false
  if count p-valids = 0 [set encerrado true]
  ifelse encerrado = true []
  [
      let trobat false
      if any? Fenomens in-radius 9[
        let f Fenomens in-radius 9
        let f1 one-of f
        face f1
        set heading heading - 180
        let patch-presa-guineu 0
        ;buscar patch huida
        let patches-huida (list 0)
        set patches-huida remove-item 0 patches-huida
        let i 0
        let d 9
        let j 0
        while [i < 24][
          if i mod 3 = 0[
            set d d - 1
            let patch-buscat patch-at-heading-and-distance heading d
            if patch-buscat != nobody[
              if  [pcolor] of patch-buscat = 65 or  [pcolor] of patch-buscat = 9 and not any? Carnivores-on patch-buscat[
                set patches-huida insert-item j patches-huida patch-buscat
                set j j + 1
              ]
            ]
          ]
          if i mod 3 = 1[
            let h heading - 45
            let patch-buscat patch-at-heading-and-distance h d
            if patch-buscat != nobody[
              if  [pcolor] of patch-buscat = 65 or  [pcolor] of patch-buscat = 9 and not any? Carnivores-on patch-buscat[
                set patches-huida insert-item j patches-huida patch-buscat
                set j j + 1
        ...
        let pathfound  false
        set i 0
        while [not pathfound and i < length patches-huida and item 0 patches-huida != 0] [
          set patch-presa-guineu item i patches-huida
      ...
      if any? Talps in-radius 9 and not trobat[
        let t Talps in-radius 9
        let tfinal sort-by [ [talp1 talp2] -> distance talp1 < distance talp2 ]  t
        let patch-presa-guineu 0
        let pathfound  false
        let i 0
        while [not pathfound and i < length tfinal ] [
          ask item i tfinal[
            set patch-presa-guineu patch-here
          ]
          let path  guineu-A* patch-guineu patch-presa-guineu p-valids
      ...
      if any? Granotes in-radius 9 and not trobat[
        let g Granotes in-radius 9
        let tfinal sort-by [ [granota1 granota2] -> distance granota1 < distance granota2 ]  g
        let patch-presa-guineu 0
        let pathfound  false
        let i 0
        while [not pathfound and i < length tfinal ] [
      ...
      if trobat = false[
        ...
        let patches-guineu-2 patches in-radius 9
        set patches-guineu-2 patches-guineu-2 with [pcolor = 65 or pcolor = 9 and not any? Carnivores-here ]
        set guineu-patches-random [ ]
        ask patches-guineu-2 [ set guineu-patches-random lput self guineu-patches-random ]
        let p position p-here guineu-patches-random
        set guineu-patches-random remove-item p guineu-patches-random

        let moviments 8
        guineu-random
        set moviments moviments - 1
        while [moviments > 0] [
          set patch-guineu patch-here
          set patches-guineu patches in-radius 9
          set p-valids patches-guineu with [pcolor = 65 or pcolor = 9 and not any? Carnivores-here]
          ifelse any? Fenomens in-radius 9[
            let f Fenomens in-radius 9
            let f1 one-of f
            face f1
            set heading heading - 180
            ...
            while [not pathfound and i < length patches-huida and item 0 patches-huida != 0] [
              ...
              let path  guineu-A* patch-guineu patch-presa-guineu p-valids
              if path != false [ ;; encontramos presa y la comemos
                set pathfound true
                set j 1 ; en la posicio 0 esta el nostre patch
                while [j < moviments  and j < length path][
                  move-to item j path
                  set j j + 1
          ...
          [
            ...
            [
              ifelse any? Granotes in-radius 9[
                let g Granotes in-radius 9
                let tfinal sort-by [ [granota1 granota2] -> distance granota1 < distance granota2 ]  g
                let patch-presa-guineu 0
                let pathfound  false
...
to guineu-random
  ...
  while [i < 4] [
    let h i * 90
    let patch-buscat patch-at-heading-and-distance h 1
    if patch-buscat != nobody[
      if member? patch-buscat guineu-patches-random [
        set patches-huida insert-item j patches-huida patch-buscat
        set j j + 1
        set entra true
      ]
  ...
  if entra[
    let p one-of patches-huida
    set guineu-patches-random remove-item position p guineu-patches-random guineu-patches-random
    move-to p

  ]
end

to-report guineu-visitats-anterior []
...
to reproduce-guineu;
  ...
  if PeriodeFertilitat != -1[
    ifelse PeriodeFertilitat = 100[
      set PeriodeFertilitat  -1
      set guineu-En-Periodo-Reproduccion remove self guineu-En-Periodo-Reproduccion ; Quitarse a si mismo de las guineus en fertilidad
    ]
    [
      Guineu-Augmenta-PeriodeFertilitat
    ]
  ]
  if (PeriodeFertilitat >= 0  and count Guineus-here > 1) or PeriodeGestacio != -1  [
    if PeriodeGestacio = -1 [ Guineu-Reset-PeriodeGestacio ]
    if PeriodeGestacio >= 0 and (PeriodeGestacio < 100)
    [
      Guineu-Augmenta-PeriodeGestacio
    ]
    if PeriodeGestacio = 100 [
      set guineu-En-Periodo-Reproduccion remove self guineu-En-Periodo-Reproduccion ; Quitarse a si mismo de las guineus en fertilidad
      set PeriodeFertilitat  -1
      set PeriodeGestacio  -1
      let mare one-of Guineus-here
      ask mare[
        hatch 2 [
          set vida 25000
          set Inanició 0
          ...
          set Cries 3
          set MaduresaCries 500
          set Madre myself   ;; El hijo se guarda a la madre(para luego seguirla)
          set EsCria true    ;; Es una cria
        ]
      ]
    ]
  ]
...
to eat-generic ;
  ...
  [
    ifelse (count Talps-here > 0)[
      let presa one-of Talps-here
      ask presa[ morir-talps myself]
      Guineu-Reset-Inanicio     ; Periode de Inanicio reiniciat
      set TempsMenjant 0
      beep
    ]
    [
...
to Guineu-Decrementa-Vida
  set Vida Vida - 1
end

to Guineu-Augmenta-Inanicio
  set Inanició Inanició + 1
end

to Guineu-Reset-Inanicio
  set Inanició 0
end

to Guineu-Augmenta-Fertilitat
  set Fertilitat Fertilitat + 1
end

to Guineu-Reset-Fertilitat
  set Fertilitat 0
end

to Guineu-Augmenta-PeriodeFertilitat
  set PeriodeFertilitat PeriodeFertilitat + 1
end

to Guineu-Reset-PeriodeFertilitat
  set PeriodeFertilitat 0
end

to Guineu-Augmenta-PeriodeGestacio
  set PeriodeGestacio PeriodeGestacio + 1
end

to Guineu-Reset-PeriodeGestacio
  set PeriodeGestacio 0
end
...
to morir-granotes [qui]
  morir
end


to morir
  set guineu-En-Periodo-Reproduccion remove self guineu-En-Periodo-Reproduccion
  set color red
  die
end
...
to-report guineu-Total-expected-cost [#Goal]
  report Cost-path + guineu-Heuristic #Goal
end

to-report guineu-Heuristic [#Goal]
  report distance #Goal
end


to-report guineu-A* [#Start #Goal #valid-map]
  ask #valid-map
  [
    set father nobody
    set Cost-path 0
  ...
  [
    ...
    ifelse any? options
    [
      ask min-one-of options [guineu-Total-expected-cost #Goal]
      [
        ...
        ask valid-neighbors
        [
          let t ifelse-value visited? [ guineu-Total-expected-cost #Goal] [2 ^ 20]
          if t > (Cost-path-father + distance myself + guineu-Heuristic #Goal)
          [
            set father myself
            set visited? true
            set active? true
  ...
  [
    ...
    While [current != #Start and current != nobody and [father] of current != current]
    [
