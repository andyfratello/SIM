...
turtles-own[ ; turtle significa qualsevol objecte que es mou en el mapa
  Vida
  Velocitat
  Inanició
  Fertilitat ; booleà 1 si es pot reproduir 0 si no es pot reproduir
  PeriodeFertilitat
  Cries
  MaduresaCries
  Digestio ; booleà que indicarà si està fent o no la digestió
  CicleDigestio
  HaParit
  tel ; puc fer una teranyina
]
...
to setup
  ...
  create-Aranyes AranyesTotal[
    ...
    set CicleDigestio 0
    set size 6
    set Vida 200 ; aquí especifico que les aranyes inicials tenen edat adulta a l'inici del programa
    set HaParit 0
    set tel 1
  ]
...
to go
  ask aranyes [
    ifelse (Vida >= 800 and Vida <= 825) or (Vida >= 1600 and Vida <= 1625) and HaParit = 0
    [
      set Fertilitat 1
    ]
    [
    ...
    set Vida Vida + 1
    if Vida > 2000 or Inanició > 1500 [die]
    ifelse Digestio = 1 and CicleDigestio < 5 [
      set CicleDigestio CicleDigestio + 1
    ...
    [ ; else
      ...
      if repr_possible = 0 and Vida <= 200 [die]
      if repr_possible = 1 [
        reproduccio-aranya
      ]
    ]
...
to moure-aranya ; moviment aranya
  ifelse(Fertilitat = 1 or (Vida <= 200) or (Vida >= 500 and Vida <= 825) or (Vida >= 1400 and Vida <= 1625)) [
    face one-of patches with [aigua = false]
  ]
  [
    if (Fertilitat = 1) [
    face one-of patches with [ aigua = false ]
    ...
    [
      if(breed = Aranyes) [
        let t one-of Granotes in-radius 15
        if Vida > 1000 and t != nobody [
          let vida_presa [vida] of t
          if vida_presa < 250 [
            face t
          ]
        ]
      ...
      if (breed = Aranyes) [
        let p one-of Papallones in-radius 15
        if p != nobody [
          face p
        ]
      ]
...
to reproduccio-aranya
  if breed = Aranyes and (count Aranyes-here > 1) and Fertilitat = 1 [
      let parella one-of Aranyes-here
      let vida_parella [vida] of parella
      if (vida_parella >= 800 and vida_parella <= 825) or (vida_parella >= 1600 and vida_parella <= 1625) and (count Aranyes-here > 1) and HaParit = 0 [
        hatch-Aranyes 100 [set Velocitat 3 set Vida 0 set Fertilitat 0]
        set Fertilitat 0
        set HaParit 1
      ]
      set Fertilitat 0
...
to menjar-aranya ; alimentacio aranya
  if breed = Aranyes and (count Papallones-here > 0) [
    if(vida >= 200) [
      set tel 0
      ask patches in-radius 2.5 [set teranyina true]
      ask patches in-radius 2.5 [set pcolor 965]
    ]
    set Inanició 0
  ...
  if breed = Aranyes and (count Granotes-here > 0) [
    let presa one-of Granotes-here
    let vida_presa [vida] of presa
    if vida_presa < 250 [
      set Inanició 0
  ...
  if breed = Aranyes and (Inanició > 1300 and count Aranyes-here > 0) [
    let presa one-of Aranyes-here
    let vida_presa [vida] of presa
    if (vida_presa < Vida and Fertilitat = 0) [
      ;me la menjo
      set Inanició 0
