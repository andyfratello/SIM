...
to setup
  ...
  create-Guineus GuineusTotal[
    ...
    while [([aigua] of (patch-at posX posY)) = true and not posFound][
       set posX random-xcor
       set posY random-ycor
    ]
    set posFound true
    ...
    set leader false
    set followingP false
    set hestating false
    set hestatingCicle 0
    set ciclesToEat 25
    set eating false
...
to guineu-buscarParella

  ifelse(partner = 0 or partner = nobody)[
    set partner one-of other turtles with [breed = Guineus and (partner = 0 or partner = nobody)] in-radius 9
    if(partner != 0 and partner != nobody)[
    let part self
    ask partner [set partner part]
    set leader true
    ask partner [guineu-DeixaDeSerLider]

    set lastEaten lastEaten + 2

  ]]
  [

    if( lastEaten  > [lastEaten] of partner)[
      set leader true
      ask partner[guineu-DeixaDeSerLider]
...
to guineu-seguirParella
  guineu-deixarDeCaçar
  if(partner != 0 and partner != nobody)[
    face partner
  ]
...
to go
  ...
  ifelse(ciclesToRep = 0)[
    if(count talps < 150 )[
    let c  (count talps / 2) * 10
      ask one-of talps[hatch-Talps c[
    set color blue
  ...
  ask Guineus[
    guineu-viure
    guineu-buscarParella

    if(partner = nobody or partner = 0)[
      set leader true]

    ifelse (not child and vida < 20200)[

   if (((vida >= 10000 and vida <= 10100) or (vida >= 20000 and vida <= 20100)) and shagged = false)[
      guineu-reproduir
    ]

   if(hestating = true and vida = hestatingCicle)[
        guineu-parir
      ]
   ifelse(leader = true)[
      guineu-buscarMenjar][
      guineu-seguirParella
    ]



      if (vida = 10100 or vida = 20100)[
        set shagged false]

      ifelse((vida >= 10000 and vida <= 10100) or (vida >= 20000 and vida <= 20100))[
        set fertil true][set fertil false]


      guineu-moure


      if( lastEaten > 200 or vida = 9999 or vida = 19999 )[
        eat-generic
...
to guineu-moure
  ...
  let movement 8
   ...
   if ((target = 0 or target = nobody) and (fertilitat = false ) and leader = true and child = false)
  [
    rt random-float 90 - random-float 90
  ]
  ...
  ifelse(target != nobody and child = true)[if (distance target <= 8 )[

    set movement distance target

    ]][
  ifelse(target != 0 and target != nobody and leader = true)[
 if (distance target <= 8 )[

    set movement distance target
    ]
  ][
  if(partner != 0 and partner != nobody and leader = false)[
     if (distance partner <= 8 )[

    set movement distance partner
    ]
  ]
  ]
]
 ...
 let found false
  ...
  while[ i <= maxMov and found = false]
  [
    let paux patch-ahead i
    if(paux != nobody)[
    if(count Carnivores-on paux != 0)[
      set found true
      set movement  i - 1
...
if(eating = false)[
    let xt xcor
    let yt ycor
    let p patch-ahead movement
    if(partner != 0 and partner != nobody and p != nobody)[
      let px [pxcor] of p
      let py [pycor] of p
      let tx [xcor] of partner
      let ty [ycor] of partner
  ...
  if(p != nobody)[
    ifelse(([aigua] of p) = false and count Carnivores-on p = 0)[
  fd movement][
      guineu-deixarDeCaçar
        rt 90]
...
to eat-generic ;
  ...
  if(count Talps-here > 0 or count Granotes-here > 0)
  [
    if (count Talps-here > 0 and count Talps-here < 5)[
...
to guineu-reproduir
  ...
  if(partner != nobody and partner != 0)[

    if( count Guineus-here > 1 and fertil = true and [fertil] of partner = true and one-of Guineus-here = partner and  shagged = false and [shagged] of partner = false)[


      set hestating true
      set hestatingCicle vida + 100
      ask partner[guineu-sexe]
      set shagged true
...
to guineu-parir
  ...
  hatch-Guineus 3[
        ...
        set hestating false
        set hestatingCicle 0
        set target one-of guineus-here with [ child = false]
        set eating false
        set ciclesToEat 25
...
to guineu-viure
  if (vida = 25000)[
    ask self[guineu-morir]
  ]
  if(lastEaten = 1000)[
    ask self[guineu-morir]]

  if(eating)[
    set ciclesToEat ciclesToEat - 1
    if(ciclesToEat = 0)[
      set eating false
      set ciclesToEat 25
      set lastEaten 0
  ...
  set vida vida + 1
  set lastEaten lastEaten + 1
end


to guineu-buscarMenjar

  ifelse(target = 0 or target = nobody)[
    set target one-of turtles with [breed != Guineus and breed != Papallones and breed != Flors  and breed != Carnivores and breed != Aranyes and breed != Fenomens] in-radius 9

    if(target != 0 and target != nobody)[
    guineu-caçar
    ]
  ]
  [ ifelse (distance target <= 9 )[
...
to guineu-caçar
  ...
  ask Guineus[if(target = targetAux and myself != self)[set taken true
