...
to setup
  ...
  create-Carnivores CarnivoresTotal[
    move-to one-of patches with [ aigua = FALSE ]
    set shape "plant"
    set color red
    set size 6
    set Vida 0
...
to go
  ...
  ask Carnivores[
    ...
    carnivora-eat
    carnivora-inanicio
    carnivora-fertilitat
  ]
  tick
end

to carnivora-envellir
  set Vida Vida + 1
  if Vida > 100000[
    let vieja one-of Carnivores-here
    ask vieja[morir-carnivora myself]
  ]
end
...
to carnivora-eat
  ...
  [
    set Inanició 0
    let presa1 one-of Papallones-here
    ask presa1[morir-papallones myself]
    beep
  ]
  if(count Aranyes-here > 0)
  [
    set Inanició 0
    let presa2 one-of Aranyes-here
    ask presa2[morir-aranyes myself]
    beep
  ]
end


to carnivora-fertilitat
  ...
  if Fertilitat = 30000[
    ...
    let Nxcor xcor
    let Nycor ycor + 5
    let NorthNeighbour one-of Carnivores at-points [[0 5]]
    let NorthPatch one-of patches at-points [[0 5]]

    let Sxcor xcor
    let Sycor ycor - 5
    let SouthNeighbour one-of Carnivores at-points [[0 -5]]
    let SouthPatch one-of patches at-points [[0 -5]]

    let Wxcor xcor - 5
    let Wycor ycor
    let WestNeighbour one-of Carnivores at-points [[-5 0]]
    let WestPatch one-of patches at-points [[-5 0]]

    let Excor xcor + 5
    let Eycor ycor
    let EastNeighbour one-of Carnivores at-points [[5 0]]
    let EastPatch one-of patches at-points [[5 0]]


    if Nycor <= max-pycor and NorthNeighbour = NOBODY [
      let agua [aigua] of NorthPatch
      if  not agua[
          hatch 1 [
            set Vida 0
            set Fertilitat 0
    ...
    if Sycor >= min-pycor and SouthNeighbour = NOBODY [
      let agua [aigua] of SouthPatch
      if  not agua[
        hatch 1 [
          set Vida 0
          set Fertilitat 0
    ...
    if Wxcor >= min-pxcor and WestNeighbour = NOBODY [
      let agua [aigua] of WestPatch
      if  not agua[
        hatch 1 [
          set Vida 0
          set Fertilitat 0
    ...
    if Excor <= max-pxcor and i < 4 and EastNeighbour = NOBODY [
      let agua [aigua] of EastPatch
      if  not agua[
        hatch 1 [
          set Vida 0
          set Fertilitat 0
...
to carnivores3[newxcor newycor maxcor]

end

to carnivora-inanicio
  set Inanició Inanició + 1
  if Inanició > 90000[
    let hambrienta one-of Carnivores-here
    ask hambrienta[morir-carnivora myself]
  ]
end

to morir-carnivora[qui]
  ifelse qui = one-of Carnivores-here[;Se mata a si misma de inanicion o de vieja
    set color red
    die
  ][
    penalitzats qui
...
to morir-papallones[qui]
  if qui = one-of Carnivores-here or qui = one-of Fenomens-here[
    set color red
    die
  ]
