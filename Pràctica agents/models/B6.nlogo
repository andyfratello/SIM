...
to setup
  ...
  ask Patches [
    ;;pxcor > 0 and pxcor < 100 and pycor > 0 and pycor < 100
    if (aiguatotal > 0)[
      set pcolor 95
      set aigua true
      set terra false
      set aiguatotal (aiguatotal - 1)
      ask patches in-radius 15 with [aigua = false] [
        set pcolor 95
        set aigua true
        set terra false
      ]
  ...
  create-Talps TalpsTotal[
    let px [0]
    let py [0]
        ask one-of patches with [aigua = false and cau = false and not any? Talps-here] [
      set px pxcor
      set py pycor
    ]
    setxy px py
  ...
  create-Carnivores CarnivoresTotal[
    let px [0]
    let py [0]
        ask one-of patches with [aigua = false and cau = false and not any? Carnivores-here] [
      set px pxcor
      set py pycor
    ]
    setxy px py
  ...
  create-Flors FlorsTotal[
    let px [0]
    let py [0]
        ask one-of patches with [aigua = false and cau = false and not any? Flors-here] [
      set px pxcor
      set py pycor
    ]
    setxy px py
  ...
  create-Aranyes AranyesTotal[
    let px [0]
    let py [0]
        ask one-of patches with [aigua = false and cau = false and not any? Aranyes-here] [
      set px pxcor
      set py pycor
    ]
    setxy px py
...
to produir-polen
  ifelse (Polinitzar < 100)[
     set Polinitzar Polinitzar + 1
  ]
  [
    ifelse (polen < 3) [
      set polen polen + 1
...
to reproduir-flor
  if (Fertilitat < 1000) [set Fertilitat Fertilitat + 1]

  if (Fertilitat = 1000) [
    if (any? patches in-radius 3 with [aigua = false and cau = false and any? Carnivores-here]) [
      hatch 1 [
          let px [0]
          let py [0]
          ask one-of patches in-radius 3 with [any? Carnivores-here] [
            set px pxcor
            set py pycor
          set pcolor black
          ]
    ...
    if (Fertilitat = 1000 and any? patches in-radius 17 with [aigua = false and cau = false and any? Carnivores-here]) [
        let px pxcor
        let py pycor
        let pxc 0
        let pyc 0

      ask one-of Patches in-radius 17 with [any? Carnivores-here] [
        set pxc pxcor
        set pyc pycor
      ]
        ask Patches in-radius 17 with [any? Carnivores-here] [

          let a (pxcor - px)
          let b (pycor - py)
          set a absolute-value(a)
          set b absolute-value(b)
          let x (a + b)


          let c (pxc - px)
          let d (pyc - py)
          set c absolute-value(c)
          set d absolute-value(d)
          let y (c + d)
      ...
      ask Patches in-radius 3 with [aigua = false and cau = false] [
    ...
    if (Fertilitat = 1000 and any? neighbors with [aigua = true]) [
    ]


    if (Fertilitat = 1000 and any? patches in-radius 17 with [aigua = true and any? neighbors with [aigua = false and cau = false]]) [
        let px pxcor
        let py pycor
        let pxc 0
        let pyc 0
        ask one-of patches in-radius 17 with [aigua = true and any? neighbors with [aigua = false and cau = false]] [
        ask one-of neighbors with [aigua = false and cau = false] [
          set pxc pxcor
          set pyc pycor
        ]
        ]
    ...
    if (Fertilitat = 1000 and any? patches in-radius 3 with [aigua = false and cau = false and not any? Flors-here]) [
      hatch 1 [
        let px [0]
        let py [0]
        ask one-of patches in-radius 3 with [aigua = false and cau = false and not any? Flors-here] [
          set px pxcor
          set py pycor
        ]
        setxy px py
    ...
    if (Fertilitat = 1000) [set color black]
...
to detectar-costa
  ask neighbors with [aigua = true and pcolor = 95] [
    set pcolor black
    detectar-costa
  ]

  ask Patches with [aigua = true and pcolor = black] [
    ask neighbors with [aigua = false and cau = false and pcolor = 65 and not any? Flors-here] [
      set pcolor white
    ]
  ;;set pcolor 95
  ]
...
to reproduir-costa
  ...
  hatch 1 [
    let px [0]
    let py [0]
    ask one-of patches in-radius 3 with [pcolor = white] [
      set px pxcor
      set py pycor
    ]
    setxy px py
