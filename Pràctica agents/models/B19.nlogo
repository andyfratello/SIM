...
to setup
  ...
  reset-ticks
   ...
   ask Patches [
    ...
    set aiguatotal aiguatotal - 900
      ...
      if (pxcor > -300 and pxcor < 300 and pycor > -300 and pycor < 300)[
      set pcolor 95
      set aigua true
      set terra false
      ask neighbors [
  ...
  create-Flors FlorsTotal[
    let patchx random-pxcor
    let patchy random-pycor
    let terraFlor false

    let probabilidadStrategia random 2

    if (probabilidadStrategia = 0) [
      ...
      if (borde = 0) [
        ...
        set patchx min-pxcor + 1
        while [terraFlor != true] [
          ask patch-at patchx patchy [
            ifelse (terra = true)[
              set terraFlor true
            ]
            [
              set patchy random-ycor
            ]
          ]
        ]
...
to go
  ...
  ask Flors [
    ...
    ifelse (CargaDePolen = 100 and count Papallones-here > 0) [
      set CargaDePolen 0
      set polen false
    ]
    [
      ifelse (CargaDePolen < 100) [
        set CargaDePolen CargaDePolen + 1
      ]
      [
        set polen true
      ]
    ...
    if (Vida = 1000 or Vida = 2000 or Vida = 3000 or Vida = 4000) [ ;inicio Reproducción

      let patch-cerca-agua false
      let posx-rep 0
      let posy-rep 0

      let ya_en_agua false
      let my_x xcor
      let my_y ycor
      let hay_agua false
      let agua_mas_cercana_x 2500
      let agua_mas_cercana_y 2500




      ask patches in-radius 4 [
       if (patch-cerca-agua = false and aigua = true) [
         ask neighbors [
           if (patch-cerca-agua = false and terra = true)[
            set patch-cerca-agua true
            ; Estrategia 3 -> moverse alrededor del agua
            set posx-rep pxcor
            set posy-rep pycor
            ]
          ]
      ...
      if (patch-cerca-agua = true) [ set Estrategia 3 ] ; iniciar Estrategia 3 (moverse alrededor del agua) si se acerca al agua
      if (xcor >= max-pxcor - 4 or xcor <= min-pxcor + 4 or ycor >= max-pycor - 4 or ycor <= min-pycor + 4) [ set Estrategia 1 ] ; iniciar Estrategia 1 si se acerca a los bordes del mapa
      if (Estrategia = 1 and ((xcor <= max-pxcor - 80 or xcor >= min-pxcor + 80) and (ycor <= max-pycor - 80 or ycor >= min-pycor + 80))) [ set Estrategia 2] ; iniciar Estrategia 2 si era Estrategia 1 y se ha separado de los bordes


      if (Estrategia = 3) [
        hatch 1 [ setxy posx-rep posy-rep set color orange set Vida 0 set polen false]
      ]
      ...
      if (Estrategia = 1) [
            ...
            ifelse(xcor + 4 > max-pxcor)[
              set posx-rep xcor - random 4
            ]
            [
              ifelse(xcor - 4 < min-pxcor)[
                set posx-rep xcor + random 4
              ]
              [
                ifelse (random 2 = 1)[
            ...
            ifelse(ycor + 4 > max-pycor)[
              set posy-rep ycor - random 4
            ]
            [
              ifelse(ycor - 4 < min-pycor)[
                set posy-rep ycor + random 4
              ]
              [
                ifelse (random 2 = 1)[
      ...
      if (Estrategia = 2) [
        ...
        hatch 1 [ setxy posx-rep posy-rep set Vida 0 set polen false]
    ...
    if (Vida = 5000) [
      hatch 1 [ setxy pxcor pycor set Vida 0 set polen false]
    ]

    if (Vida = 5000  or [cau] of patch-here) [
      morir
    ]
...
to morir-flors [qui]
  ifelse ([breed] of qui = Talps and [cau] of patch-here) [
    set color red
    die
  ]
  [
